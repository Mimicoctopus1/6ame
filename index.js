...
io.on('connection', function (socket) {
  socket.on("getCurrentToS", function() {
    ...
    console.log(readJSON("game.json")[socket.username]["moves"]);
    if(readJSON("game.json")[socket.username]["moves"][messageWords[1]]) {/*If the command is in the user's move property (if the user knows the move).*/
